...
Array.prototype.chunkBy = function (step, key) {
  ...
  for (let i = (r = j = c = 0); i < this.length; ) {
    ...
    if (future > step) {
      // if future stack higher than chunk size
      ...
      // if future stack lesser than chunk size
...
app.post("/attributes", async (req, res) => {
  ...
  await queryDB(async () => {
    ...
    await attributes.updateOne(query, {
      $push: { data: { name: req.body.name, code: req.body.code } },
    });
  });
});
...
app.post("/stock", async (req, res) => {
  // accept also changes on the stock table
...
app.get("/press", async (req, res) => {
  const toBePrinted = await queryDB(async () => {
    ...
    const stc = await stock
      .find(query)
      .project({ _id: 1, code: 1, quantity: 1, printed: 1 })
      .toArray();
    return stc;
  });
  const chunked = toBePrinted.chunkBy(18, "quantity");
  /* mammoth
    .convertToHtml({ path: "./label.docx" })
    .then(async (res) => {
...
app.post("/press", (req, res) => {
  ...
  const toBePrinted = req.body;
  const cache = {};
  const chunked = toBePrinted.chunkBy(3, "quantity");
  cooked = ``;
  chunked.forEach((chunk, ch_i) => {
    cooked += `<tr style="page-break-inside: avoid; height: 132.1pt">`;
    chunk.forEach((data, da_i) => {
      cooked += `
      <td style="width: 180pt; padding: 0cm 0.75pt 0cm 0.75pt; height: 132.1pt;">
        <p class="MsoNormal" style="
        margin-top: 0cm;
        margin-right: 4.5pt;
        margin-bottom: 0cm;
        margin-left: ${!da_i ? "0pt" : "4.5pt"};
        margin-bottom: 0.0001pt;
        text-align: center;
        ">
          <span>
            <img height="165" src="./default.jpg" />
          </span>
        </p>
      </td>`;
      cooked += `
      ...
      </td>`.repeat(data.quantity - 1);
    });
    cooked += `</tr>`;
  });
  let parsed = fs.readFileSync("./structure.htm").toString();
  parsed = parsed.replace(/{{insert}}/, cooked);
  fs.writeFileSync("./test.html", parsed);
